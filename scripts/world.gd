...
func _create_materials() -> void:
	for raw_id in GameRegistry.BLOCKS.keys():
		var block_id: String = str(raw_id)
		if block_id == GameRegistry.AIR:
			continue
		materials[block_id] = _make_material(GameRegistry.block_color(block_id))
...
func _position_player_safely() -> void:
	if player_ref == null:
		return
	var sx: int = 8
	var sz: int = 8
	var height: int = int(height_map.get(Vector2i(sx, sz), 6))
	if player_ref.global_position.y < float(height + 2):
		player_ref.global_position = Vector3(float(sx) + 0.5, float(height) + 2.0, float(sz) + 0.5)

func _player_chunk_coord() -> Vector2i:
	if player_ref == null:
		return Vector2i.ZERO
	var bx: int = floori(player_ref.global_position.x / BLOCK_SIZE)
	var bz: int = floori(player_ref.global_position.z / BLOCK_SIZE)
...
func _update_chunks(force: bool) -> void:
	...
	for raw_coord in loaded_chunks.keys():
		var chunk_coord: Vector2i = raw_coord
		var dist: int = max(abs(chunk_coord.x - center.x), abs(chunk_coord.y - center.y))
...
func _generate_chunk(coord: Vector2i) -> void:
	...
	for lx in range(CHUNK_SIZE):
		for lz in range(CHUNK_SIZE):
			var x: int = origin_x + lx
			var z: int = origin_z + lz
			var biome_id: String = GameRegistry.biome_at(x, z)
			var height: int = _height_for_column(x, z, biome_id)
			height_map[Vector2i(x, z)] = height
			for y in range(height):
				var pos: Vector3i = Vector3i(x, y, z)
				var placed: bool = set_block(pos, _block_for_depth(x, y, z, height, biome_id))
				if placed:
					block_list.append(pos)

	var tree_blocks: Array[Vector3i] = _generate_trees_in_chunk(origin_x, origin_z)
	block_list.append_array(tree_blocks)

	if coord == Vector2i.ZERO:
		var marker_blocks: Array[Vector3i] = _spawn_marker_blocks()
		block_list.append_array(marker_blocks)
		if not bonus_spawned:
			bonus_spawned = true
			_spawn_bonus_items()
...
func _unload_chunk(coord: Vector2i) -> void:
	...
	for raw_position in block_list:
...
func _height_for_column(x: int, z: int, biome_id: String) -> int:
	var wave: float = sin(float(x) * 0.31) + cos(float(z) * 0.27) + sin(float(x + z) * 0.14) + cos(float(x - z) * 0.10)
	var base_height: int = 4 + int(round(wave * 1.25))
	if biome_id == "dunes":
		base_height -= 1
...
func _block_for_depth(x: int, y: int, z: int, height: int, biome_id: String) -> String:
	...
	var ore: String = _ore_for_depth(x, y, z)
	if ore != "":
		return ore
	if y <= 1:
		return "deepstone"
...
func _ore_for_depth(x: int, y: int, z: int) -> String:
	var roll: int = int(abs(sin(float(x * 19 + z * 31 + y * 47)) * 10000.0)) % 100
	if y <= 3 and roll < 9:
...
func _generate_trees_in_chunk(origin_x: int, origin_z: int) -> Array[Vector3i]:
	var placed: Array[Vector3i] = []
	for lx in range(CHUNK_SIZE):
		for lz in range(CHUNK_SIZE):
			var x: int = origin_x + lx
			var z: int = origin_z + lz
			var column: Vector2i = Vector2i(x, z)
			if not height_map.has(column):
				continue
			var biome_id: String = GameRegistry.biome_at(x, z)
			if not GameRegistry.biome_allows_trees(biome_id):
				continue
			if int(abs(sin(float(x * 71 + z * 97)) * 1000.0)) % 13 != 0:
				continue
			var ground_y: int = int(height_map[column])
			for trunk_y in range(ground_y, ground_y + 4):
				var trunk_pos: Vector3i = Vector3i(x, trunk_y, z)
				if set_block(trunk_pos, "oak_log"):
					placed.append(trunk_pos)
			for tlx in range(-2, 3):
				for tlz in range(-2, 3):
					if abs(tlx) + abs(tlz) <= 3:
						var leaf_pos: Vector3i = Vector3i(x + tlx, ground_y + 4, z + tlz)
						if set_block(leaf_pos, "leaf_block"):
							placed.append(leaf_pos)
			var top_pos: Vector3i = Vector3i(x, ground_y + 5, z)
...
func _spawn_marker_blocks() -> Array[Vector3i]:
	...
	for entry in markers:
		...
		var block_id: String = str(entry[1])
...
func set_block(block_position: Vector3i, block_id: String) -> bool:
	...
	if blocks.has(block_position):
		remove_block(block_position)
	var block_data: Dictionary = GameRegistry.get_block(block_id)
...
func break_block_from_collider(collider: Object) -> Dictionary:
	...
	var block_id: String = str(collider.get_meta("block_id"))
	remove_block(block_position)
	var coord: Vector2i = Vector2i(floori(float(block_position.x) / float(CHUNK_SIZE)), floori(float(block_position.z) / float(CHUNK_SIZE)))
...
func place_block_from_hit(hit_position: Vector3, hit_normal: Vector3, block_id: String) -> bool:
	if block_id == "" or not GameRegistry.BLOCKS.has(block_id):
		return false
	var target: Vector3i = world_to_block(hit_position + hit_normal * 0.51)
	if target.y < WORLD_FLOOR or target.y > 80:
		return false
	if has_block(target):
		return false
	var placed: bool = set_block(target, block_id)
	if placed:
		var coord: Vector2i = Vector2i(floori(float(target.x) / float(CHUNK_SIZE)), floori(float(target.z) / float(CHUNK_SIZE)))
		if chunk_nodes.has(coord):
			var list: Array = chunk_nodes[coord]
			list.append(target)
	return placed
...
func spawn_pickup(item_id: String, amount: int, pos: Vector3) -> void:
	var pickup: PickupItem = PickupItem.new()
